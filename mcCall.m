function [I_hat,IC_h,IC_b,Op_exact]=mcCall(pas,N_t,alpha)
    ...
    % simulation de pas*(N_t+1) variables normales centrées
    % reduites
    X=randn(1,pas*(N_t+1));
    
    % on calcule le vecteur des valeurs de l'option en
    % vectorisant les calculs
    OP_sim=max(exp(beta*X)-K,0);
    ...
    for i=1:N_t
        ...
        n=n+pas;
        S1=S1+sum(OP_sim(i*pas+1:n));
        S2=S2+sum(OP_sim(i*pas+1:n).^2);
